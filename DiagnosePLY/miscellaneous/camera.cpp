...
void Camera::reset() {
  ...
  mFrame.orientation.setIdentity();
  setTarget(Vector3f::Zero());
}

void Camera::setViewport(int offsetx, int offsety, int width, int height) {
  mVpX = offsetx;
  mVpY = offsety;
  mVpWidth = width;
  mVpHeight = height;
...
Vector3f Camera::unProject(const Vector2f &uv, float depth, const Matrix4f &invModelview) const {
  ...
  Vector3f a(2.0f * uv.x() / float(mVpWidth) - 1.0f, 2.0f * uv.y() / float(mVpHeight) - 1.0f, 1.0f);
